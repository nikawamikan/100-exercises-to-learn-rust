...
私が知る限り、他の言語では例外処理は言語仕様に埋め込まれていることが多いが、RustではEnumを利用して例外処理を実装している。  
これは、Rustの哲学である「ゼロコスト抽象化」に基づいているのだと思われる。

Enumを利用することで、この例外処理(厳密には例外処理ではないが)はコンパイル時に解決されるため、最適化されたコードを生成することができるのが面白い特徴である。
